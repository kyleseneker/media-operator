...
func init() {
	...
	utilruntime.Must(indexersv1alpha1.AddToScheme(scheme))
}
...
var allControllers = []controllerFactory{
	{name: "prowlarr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &indexers.ProwlarrConfigReconciler{
	...
	{name: "flaresolverr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &indexers.FlareSolverrConfigReconciler{
			Client:   mgr.GetClient(),
			Scheme:   mgr.GetScheme(),
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "media-operator-indexers.media-operator.dev",
		Cache:                  cacheOptions,
	})
	if err != nil {
