...
func init() {
	...
	utilruntime.Must(pvrv1alpha1.AddToScheme(scheme))
}
...
var allControllers = []controllerFactory{
	{name: "sonarr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &pvr.SonarrConfigReconciler{
	...
	{name: "radarr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &pvr.RadarrConfigReconciler{
			Client:   mgr.GetClient(),
			Scheme:   mgr.GetScheme(),
	...
	{name: "lidarr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &pvr.LidarrConfigReconciler{
			Client:   mgr.GetClient(),
			Scheme:   mgr.GetScheme(),
	...
	{name: "readarr", setup: func(mgr ctrl.Manager) controllerSetup {
		return &pvr.ReadarrConfigReconciler{
			Client:   mgr.GetClient(),
			Scheme:   mgr.GetScheme(),
...
func main() {
	...
	flag.StringVar(&enableControllers, "enable-controllers", "*",
		"Comma-separated list of controllers to enable (e.g., sonarr,radarr,prowlarr). Use * for all.")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "media-operator-pvr.media-operator.dev",
		Cache:                  cacheOptions,
	})
	if err != nil {
