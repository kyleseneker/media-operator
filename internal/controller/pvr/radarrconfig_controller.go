...
func (r *RadarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	cleanup := func(ctx context.Context) error {
		return ctrlcommon.CleanupApp(ctx, r.Client, r.Recorder, &config, config.Spec.Connection, pvrclient.Definition("v3"), "radarr")
	}
	...
	hc, err := engine.NewHTTPClient(config.Spec.Connection.URL, engine.AuthAPIKey, engine.WithOwner(config.Namespace+"/"+config.Name+"/"+string(config.UID)), engine.WithAPIKey(apiKey), engine.WithTLSConfig(tlsCfg), engine.WithAppLabel("radarr"))
	if err != nil {
...
func (r *RadarrConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&pvrv1alpha1.RadarrConfig{}, builder.WithPredicates(ctrlcommon.ConfigChangedPredicate())).
