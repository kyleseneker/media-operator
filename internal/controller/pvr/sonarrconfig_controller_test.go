...
func newTestReconciler(t *testing.T, objs ...runtime.Object) *SonarrConfigReconciler {
	...
	c := fake.NewClientBuilder().
		...
		WithStatusSubresource(&pvrv1alpha1.SonarrConfig{}).
		Build()
...
func sonarrConfig(url string, mutate func(*pvrv1alpha1.SonarrConfig)) *pvrv1alpha1.SonarrConfig {
	cfg := &pvrv1alpha1.SonarrConfig{
		ObjectMeta: metav1.ObjectMeta{Name: "sonarr", Namespace: "media"},
		Spec: pvrv1alpha1.SonarrConfigSpec{
...
func getConfig(t *testing.T, r *SonarrConfigReconciler) *pvrv1alpha1.SonarrConfig {
	t.Helper()
	var cfg pvrv1alpha1.SonarrConfig
...
func conditionStatus(cfg *pvrv1alpha1.SonarrConfig, condType string) string {
...
func TestReconcileWritesDeclaredResources(t *testing.T) {
	...
	rename := true
	r := newTestReconciler(t, apiKeySecret(), sonarrConfig(srv.URL, func(c *pvrv1alpha1.SonarrConfig) {
		c.Spec.Naming = &pvrv1alpha1.SonarrNaming{
...
func TestPruneRemovesPreviouslyManagedResources(t *testing.T) {
	...
	enabled := true
	cfg := sonarrConfig(srv.URL, func(c *pvrv1alpha1.SonarrConfig) {
		c.Spec.Reconcile = &commonv1alpha1.ReconcileConfig{Prune: &enabled}
		c.Spec.DownloadClients = []commonv1alpha1.DownloadClient{{
			Name: "qBittorrent", Protocol: "torrent", Implementation: "QBittorrent",
			Host: "qbittorrent", Port: 8080, Category: "tv",
